...
import { fetchFeaturedBooks, deleteBook, fetchBooks } from '@/redux/features/book/slice';
...
const BookshelfMenu = () => {
    ...
    const handleKeyPress = (event) => {
        if (event.key === 'Enter') {
            ...
            let filteredReadingBooks = books?.filter(book =>
            ...
            let filteredPlanToReadBooks = books?.filter(book =>
            ...
            let filteredCompletedBooks = books?.filter(book =>
                        ...
                        <div className="overflow-x-auto max-w-full" >
                            <div className="grid grid-cols-3 gap-4" >
                                {filteredPlanToReadBooks !== null ?
                                    ...
                                    books?.filter((book) => book.status === "Plan to Read")?.map((book) => (
                                        <Bookcard key={book._id} book={book}
                                        />
                                    ))
                                }
                            </div>
                            {!books?.filter((book) => book.status === "Plan to Read")?.length && (
                                <div className="mt-12 text-gray-600">
                                    No Books Found
                                </div>)}
